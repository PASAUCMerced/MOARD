...
void analysis_on_operand(instrStateUnit *an_instrStateUnit, vector<an_intrStUni *> subTarData, vector<instrInfo*> app_trace, int sum, int sz, string sign, int k, string signSubTaData, int opcodeId, arguConfigSpace *an_arguConfig, globalStateUnit *an_globalStateUnit, fmCounter *fmc, ifstream &isfile, ofstream &isfile1);    
...
void analysis_on_operand(instrStateUnit *an_instrStateUnit, vector<an_intrStUni *> subTarData, vector<instrInfo*> app_trace, int sum, int sz, string sign, int k, string signSubTaData, int opcodeId, arguConfigSpace *an_arguConfig, globalStateUnit *an_globalStateUnit, fmCounter *fmc, ifstream &isfile, ofstream &isfile1)
{
	...
	int nOfSteps = 1;
#ifdef LESS_DETAILS_VERSION
	if(tmp_ratio < 1)  //Dong's comment: ratio > 1 is illegal;
#else
	if(tp->ratio <1)
#endif
	{
...
#endif
		{
			if (isfile){
				...
				while(!isfile.eof())
				{
					...
					if((an_inFaInject.subTarData == signSubTaData)&&(an_inFaInject.dynId == app_trace[k]->line0_dynInstId))
					{
						if((an_inFaInject.opId == app_trace[k]->line0_opcodeId)&&(an_inFaInject.regName == subTarData[0]->name)&&(an_inFaInject.lyId == app_trace[k]->line0_lineId))
						{
...
int main()
{
	...
	ifstream isfile;
	isfile.open("../midd_file/fault_injection_rel");//Anzheng's comment: add index here
	ofstream isfile1;
	isfile1.open("../midd_file/fault_injection");//Anzheng's comment: add index here
	...
	string filename_tmp = "../tracefile/dynamic_trace";	//Anzheng's comment; add index here
	...
	while(true)
	{
		...
		if (yes_no == string::npos)
		{
			...
			if ((n>=an_arguConfig->NUMBER_OF_BLOCKS_PROCESS_PER_BATCH)||in.eof())
			{
				...
				for(k=0;k<len;k++)
				{
					...
					if (opcodeId < 0)
					{
						printf("Exception: disaster found %d!", app_trace[k]->line0_dynInstId.c_str());
					}
					...
					{
						signSubTaData = "1";
						analysis_on_operand(an_instrStateUnit, an_instrStateUnit->subTarData1, app_trace, sum, sz1, sign, k, signSubTaData, opcodeId, an_arguConfig, an_globalStateUnit, fmc, isfile, isfile1);
					}
					if(!an_instrStateUnit->subTarData2.empty())
					{
						signSubTaData = "2";
						analysis_on_operand(an_instrStateUnit, an_instrStateUnit->subTarData2, app_trace, sum, sz1, sign, k, signSubTaData, opcodeId, an_arguConfig, an_globalStateUnit, fmc, isfile, isfile1);
					}
					if(!an_instrStateUnit->subTarData3.empty())
					{
						signSubTaData = "3";
						analysis_on_operand(an_instrStateUnit, an_instrStateUnit->subTarData3, app_trace, sum, sz1, sign, k, signSubTaData, opcodeId, an_arguConfig, an_globalStateUnit, fmc, isfile, isfile1);
					}
					...
					if(atoi(app_trace[k]->line0_dynInstId.c_str())%500 == 0)
						printf("The traceAnalysisTool has completed the %d instructions...\n", atoi(app_trace[k]->line0_dynInstId.c_str()));
                    			...
                    			if (atoi(app_trace[k]->line0_dynInstId.c_str())  > 608730)
						printf("The traceAnalysisTool has completed the %d instructions...\n", atoi(app_trace[k]->line0_dynInstId.c_str()));

					if (app_trace[k]->line0_opcodeId == 608738)
					{
						printf("The traceAnalysisTool has completed the %d instructions...\n", atoi(app_trace[k]->line0_dynInstId.c_str()));
					}
		...
		{
		    printf("starting to read the trace information ... \n");
			//NumOfComma1 = 1;
	...
	cout<<"The running time of the program is:"<<totaltime1<<"(s)"<<endl;
	totaltime2 = totaltime1/60;
	cout<<"The running time of the program is:"<<totaltime2<<"(min)"<<endl;
	totaltime3 = totaltime2/60;
	cout<<"The running time of the program is:"<<totaltime3<<"(h)"<<endl;
...
#ifndef LESS_DETAILS_VERSION
	ofstream filean;
	filean.open("../analysis_result/detailed_fm_events_info", ofstream::out);
	...
	ofstream anfile;
	anfile.open("../analysis_result/dvf_counters", ofstream::out); //Anzheng's comment: add index here
#ifdef LESS_DETAILS_VERSION
	...
	for (int i = 0; i < 150; i++)
		...
		anfile<<endl;
		anfile<<"The running time of the program is:"<<totaltime1<<"(s)"<<endl;
	    anfile<<"The running time of the program is:"<<totaltime2<<"(min)"<<endl;
    	anfile<<"The running time of the program is:"<<totaltime3<<"(h)"<<endl;
