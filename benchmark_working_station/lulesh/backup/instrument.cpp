...
int functionDynInstCounter(int counter, char* OpcodeName, int phiCounter)
{
  dynInstCounter++;
  dynInstCounter=dynInstCounter+phiCounter;
...
int intFaultInjection(int c_value, int dynInstrID,int bitPosition){
  ...
  w = 1;
  if (abs(dynInstrID-dynInstCounter)<50){
    cout << "Start doing fault injection into the correct position ..."<<endl;
    w<<=bitPosition;
	c_value = c_value^w;
	return c_value;
  }else{
